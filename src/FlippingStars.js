...
const FlippingStars = ({ onClose, setShowProfileView, setActiveTab }) => {
  ...
  // Daily member task states - COMMENTED OUT
  ...
  // Get daily member task API - COMMENTED OUT
  ...
  // Complete daily member task API - COMMENTED OUT
    ...
    const performAutoFlip = async () => {
      ...
      manualFlipTimerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (result.success) {
            // Handle daily member task completion on first valid flip of session (premium users only) - COMMENTED OUT
            ...
            // Set last bet amount for floating animation
  ...
  return (
    <div className={`fc_app ${isAutoFlipping ? 'is-auto-flipping' : ''}`}>
      ...
      <div className={`fc_logo-container ${show3D ? 'fc_logo-elevated' : ''} ${logoImage !== flippingStarsLogo ? 'fc_logo-result' : ''}`}>
        ...
        {(!isFlipping || isAutoFlipping) && isAnimating && ((isPositiveChange && winReward > 0) || (!isPositiveChange && lastBetAmount > 0)) && (
