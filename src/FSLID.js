...
const FSLID = () => {
    ...
    // Helper function to build FSL login URL manually for production
    const buildFSLLoginUrl = (state, redirectUrl) => {
        const fslLoginUrl = new URL('https://id.fsl.com/login/fslUsers');
        fslLoginUrl.searchParams.append('response_type', 'code');
        fslLoginUrl.searchParams.append('appkey', 'yshgTgd6qz9');
        fslLoginUrl.searchParams.append('scope', 'basic,wallet,stepn');
        fslLoginUrl.searchParams.append('state', state);
        fslLoginUrl.searchParams.append('is_app', '0');
        fslLoginUrl.searchParams.append('withState', '1');
        fslLoginUrl.searchParams.append('use_popup', '0');
    ...
    const connectFSLID = async () => {
        ...
        try {
            ...
            if (isLineEnvironment()) {
                ...
                // Build FSL login URL manually for production
                const loginUrl = buildFSLLoginUrl(state, REDIRECT_URL);
                console.log('FSL Login URL for LINE (production):', loginUrl);
